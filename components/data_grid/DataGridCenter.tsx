...
const DataGridCenter = () => {
    ...
    const TabButton = ({ tabName, label }: { tabName: Tab; label: string }) => (
        ...
        >
            {label}
            {activeTab === tabName && (
                <motion.div className="absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-500" layoutId="dataGridTab" />
            )}
        </button>
    );
