...
const ConversationSettingsModal = ({ onClose }: { onClose: () => void; }) => {
    ...
    return (
        <MotionDiv initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <MotionDiv initial={{scale:0.95}} animate={{scale:1}} className="bg-gray-800 w-full max-w-md rounded-xl shadow-2xl overflow-hidden" onClick={(e: any) => e.stopPropagation()}>
