...
const VersionSystem = () => {
    ...
    return (
        <>
            <button 
                onClick={handleClick}
                className="relative group hidden md:flex items-center justify-center gap-2 px-3 py-1.5 rounded-full bg-gray-800 border border-gray-700 hover:border-indigo-500 hover:bg-gray-750 transition-all cursor-pointer mr-2"
        ...
        <header className="relative w-full glass-panel z-30 px-4 py-2 border-b border-white/5 flex-shrink-0 safe-top">
            <div className="flex items-center justify-between w-full max-w-5xl mx-auto gap-4">
                <div className="flex items-center gap-3">
                    <button onClick={() => setConversationPanelOpen(prev => !prev)} className="p-2 text-gray-400 hover:bg-gray-800 rounded-lg transition-colors">
                    ...
                    <div className="min-w-0 flex-1">
                        {isEditing ? (
                            <input
                                ...
                                className="bg-gray-800 text-white rounded-md px-2 py-0.5 text-sm font-bold w-full focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                                autoFocus
                            />
                        ) : (
                            <h1 
                                onClick={() => setIsEditing(true)} 
                                className="text-sm font-bold truncate text-gray-200 cursor-pointer hover:text-indigo-400 transition-colors"
                            >
