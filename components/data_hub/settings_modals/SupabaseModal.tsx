...
export const SupabaseModal = ({ service, onClose, onSaveSuccess }: ModalProps) => {
    const {
        config,
        handleConfigChange,
        isSaving,
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-gray-800/80 backdrop-blur-lg rounded-lg shadow-xl w-full max-w-2xl border border-white/10" onClick={e => e.stopPropagation()}>
                ...
                <main className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <div>
                        ...
                        <input type="text" value={config.projectUrl || ''} onChange={e => handleConfigChange('projectUrl', e.target.value)} placeholder="https://<project-ref>.supabase.co" className="w-full p-2 mt-1 bg-gray-700 rounded-lg text-sm" />
                    </div>
                    <div>
                        <label className="text-sm font-medium text-gray-300">Anon (Public) Key</label>
                        <div className="relative mt-1">
                            <input type={isKeyVisible ? 'text' : 'password'} value={config.anonKey || ''} onChange={e => handleConfigChange('anonKey', e.target.value)} placeholder="Your Supabase anon key" className="w-full p-2 pr-10 bg-gray-700 rounded-lg text-sm font-mono"/>
                            <button onClick={() => setIsKeyVisible(!isKeyVisible)} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white">{isKeyVisible ? <EyeSlashIcon className="w-5 h-5"/> : <EyeIcon className="w-5 h-5" />}</button>
                        </div>
                    </div>
                    <div className="border-t border-gray-700/50 pt-4">
                        ...
                        <div className="bg-gray-900 rounded-lg p-2 mt-2 h-32 overflow-y-auto text-xs font-mono">
                            {connectionLog.length > 0 ? connectionLog.map((entry, i) => (<p key={i} className={`flex items-start gap-2 ${entry.status === 'success' ? 'text-green-400' : entry.status === 'error' ? 'text-red-400' : 'text-gray-400'}`}><span>[{entry.timestamp}]</span> - <span>{entry.message}</span></p>)) : <p className="text-gray-500 text-center pt-8">Ready to test connection.</p>}
                        </div>
                    </div>
                </main>
