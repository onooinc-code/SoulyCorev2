...
export const VercelPostgresModal = ({ service, onClose }: { service: DataSource, onClose: () => void }) => {
    const [connectionString, setConnectionString] = useState('postgres://user:pass@host:port/db');
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-gray-800 rounded-lg shadow-xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 space-y-4">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 mt-1">Found in your Vercel project's storage settings.</p>
                    </div>
                </div>
