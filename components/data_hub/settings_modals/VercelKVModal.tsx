...
export const VercelKVModal = ({ service, onClose, onSaveSuccess }: ModalProps) => {
    const {
        config,
        handleConfigChange,
        isSaving,
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-gray-800/80 backdrop-blur-lg rounded-lg shadow-xl w-full max-w-2xl border border-white/10" onClick={e => e.stopPropagation()}>
                ...
                <main className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <div>
                        ...
                        <input type="text" value={config.url || ''} onChange={e => handleConfigChange('url', e.target.value)} placeholder="https://..." className="w-full p-2 mt-1 bg-gray-700 rounded-lg text-sm" />
                    </div>
                     <div>
                        <label className="text-sm font-medium text-gray-300">REST API Token (Read/Write)</label>
                        <div className="relative mt-1">
                            <input type={isTokenVisible ? 'text' : 'password'} value={config.token || ''} onChange={e => handleConfigChange('token', e.target.value)} placeholder="Your Read/Write Token" className="w-full p-2 pr-10 bg-gray-700 rounded-lg text-sm" />
                     ...
                     <div>
                        ...
                        <div className="relative mt-1">
                            <input type={isReadOnlyTokenVisible ? 'text' : 'password'} value={config.readOnlyToken || ''} onChange={e => handleConfigChange('readOnlyToken', e.target.value)} placeholder="Your Read-Only Token" className="w-full p-2 pr-10 bg-gray-700 rounded-lg text-sm" />
                            <button onClick={() => setIsReadOnlyTokenVisible(!isReadOnlyTokenVisible)} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white">{isReadOnlyTokenVisible ? <EyeSlashIcon className="w-5 h-5"/> : <EyeIcon className="w-5 h-5" />}</button>
                        </div>
                    </div>
