...
export const VercelBlobModal = ({ service, onClose }: { service: DataSource, onClose: () => void }) => {
    const [connectionString, setConnectionString] = useState('YOUR_BLOB_CONNECTION_STRING');
    const [testStatus, setTestStatus] = useState<'idle' | 'testing' | 'success' | 'error'>('idle');

    const handleTest = () => {
        setTestStatus('testing');
        setTimeout(() => {
            setTestStatus(connectionString ? 'success' : 'error');
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4" onClick={onClose}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-gray-800 rounded-lg shadow-xl w-full max-w-lg" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 space-y-4">
                    <div>
                        <label htmlFor="connString" className="block text-sm font-medium text-gray-400 mb-1">Connection String</label>
                        <input type="password" id="connString" value={connectionString} onChange={e => setConnectionString(e.target.value)} className="w-full p-2 bg-gray-700 rounded-lg text-sm font-mono focus:ring-2 focus:ring-indigo-500 focus:outline-none" />
                        <p className="text-xs text-gray-500 mt-1">Found in your Vercel project's storage settings for Blob.</p>
                    </div>
                </div>
