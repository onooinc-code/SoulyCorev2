...
export const VercelBlobModal = ({ service, onClose, onSaveSuccess }: ModalProps) => {
    const {
        config,
        handleConfigChange,
        isSaving,
                ...
                <main className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                     <div>
                        <label className="text-sm font-medium text-gray-300">BLOB_READ_WRITE_TOKEN</label>
                         <p className="text-xs text-gray-500 mb-2">This token grants read and write access to your Vercel Blob store.</p>
                        <div className="relative">
                            <input type={isTokenVisible ? 'text' : 'password'} value={config.token || ''} onChange={e => handleConfigChange('token', e.target.value)} placeholder="vercel_blob_rw_..." className="w-full p-2 pr-10 bg-gray-700 rounded-lg text-sm" />
