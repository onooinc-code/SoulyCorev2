...
export const VercelBlobModal = ({ service, onClose }: ModalProps) => {
    const { log } = useLog();
    ...
    const handleTestConnection = useCallback(async () => {
        ...
        log('Testing Vercel Blob connection...');
        const newLogEntry: ConnectionLog = {
            timestamp: new Date(),
            status: 'error',
            message: 'Connection failed. Invalid token format.',
        };

        await new Promise(resolve => setTimeout(resolve, 1500));
        
        const success = token.startsWith('vercel_blob_rw_');
        
        if (success) {
            newLogEntry.status = 'success';
            newLogEntry.message = 'Connection successful. Token is valid.';
        }

        setConnectionStatus(newLogEntry.status);
        setConnectionLogs(prev => [newLogEntry, ...prev]);
        log(`Vercel Blob connection test result: ${newLogEntry.status}`);

    }, [token, log]);

    const handleSave = async () => {
        setIsSaving(true);
        log('Saving Vercel Blob settings...');
        ...
        >
            ...
            >
                ...
                <main className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                     <div>
                        <label className="text-sm font-medium text-gray-300">BLOB_READ_WRITE_TOKEN</label>
                         <p className="text-xs text-gray-500 mb-2">This token grants read and write access to your Vercel Blob store. It will be stored securely.</p>
                        <div className="relative">
                            <input type={isTokenVisible ? 'text' : 'password'} value={token} onChange={e => { setToken(e.target.value); setIsModified(true); }} placeholder="vercel_blob_rw_..." className="w-full p-2 pr-10 bg-gray-700 rounded-lg text-sm" />
