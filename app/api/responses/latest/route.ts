...
export async function GET() {
    try {
        // FIX: Replaced `process.cwd()` with `path.resolve()` to resolve a TypeScript error where the `cwd` method was not found on the `Process` type.
        // Both methods return the current working directory, so this change maintains the original logic while fixing the type issue.
